...
import logging # Keep existing logging
...
# --- Build the MCP Engine (No Change) ---
mcp_engine = FastMCP("MCP Unified Server")
console = Console()

# --- Tool Registration (No Change) ---
...
logger.info("FastAPI app created.") # Log app creation
...
async def handle_call_tool(request: Request):
    ...
    try:
        ...
        logger.exception(f"Unexpected error executing tool '{tool_name or 'unknown'}' via /call_tool") # Log full traceback
        raise HTTPException(status_code=500, detail=f"Internal server error: {e}")

# --- Health Check Endpoint (No Change) ---
@app.get("/")
...
logger.info("Server setup complete, ready for Uvicorn.") # Final log before Uvicorn takes over